...
<WorkflowBlock>
  ...
  <WorkflowBlockItem>
    Choose a template for your report from the options you have configured.\* **Blocked Device**s - Report on devices currently blocked access by Sentry.
  ...
  <WorkflowBlockItem>
    Click **Next**.The **Report Details** page is displayed.\* Enter a **Report Name**.

    - (Optional) Enter a **Description** for the report.Select the **Event Range** from the following options\:For existing reports:
  </WorkflowBlockItem>
  ...
  <WorkflowBlockItem>
    - CSV
    - PDF
    - **CSV and PDF**For PDF report files, up to 10 columns are allowed. In the Report Charts section, two types of charts that will be included in the PDF reports are displayed.**All Applications** report supports only the CSV format.
...
- To assign a custom role to a user, see [Assigning Roles](Assigning_Roles_to_Users.mdx).
- See [User Roles](User_Roles.mdx) for a list of default roles.
