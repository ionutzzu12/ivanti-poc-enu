...
- [Adding Users](Adding_Users.mdx)
- [Adding multiple users](Adding_Users.mdx)
- [Adding multiple users by uploading a file](Adding_Users.mdx)
- [Adding an administrator](Adding_Users.mdx)
- [Nobody user](Adding_Users.mdx)
- [Viewing the device registration PIN information](Adding_Users.mdx)

You can add a single user or several users at a time. Once you have added many users, you might want to [filter](Finding_and_Filtering_Users.mdx) the display to show only the ones you are interested in.

Other things you can do with users in this page include:

- [assign](Assigning_Users_to_User_Groups.mdx) to/ [remove](Removing_Users_from_User_Groups.mdx) from a user group
- [send a message](Sending_a_Message.mdx)
- [invite to register](Inviting_Users.mdx)
- [assign roles](Assigning_Roles_to_Users.mdx)
- [change a password](Changing_a_Password.mdx)
- [delete](Deleting_a_User.mdx)
- [generate a pin](Device_Registration_by_PIN_Generation.mdx)
...
4. Complete the form with the user's information:\* Email Address
   - First Name
   - Last NameThe Username field displays the email address you entered. In most cases, you should not edit this default. For more information, see [When to Edit a Username](Editing_a_Username.mdx)\* Display NameIf you want to change the display name for this user, edit the default text in the **Display Name** field.
...
6. Edit the template with the following information for each user:\* user ID (required)
   ...
   - user groups
   - custom attributes
7. This is the same information you enter when [adding a single user](Adding_Users.mdx). Do not exceed 10,000 entries in the file.
   Save the file.
