...
- [Editing the default setting](User_Settings.mdx)
- [Adding a custom setting](User_Settings.mdx)
- [Deleting a custom setting](User_Settings.mdx)
- [Configuring the settings for new device registrations](User_Settings.mdx)
- [Configuring the device limit per user](User_Settings.mdx)
- [Configuring the device wipe limit](User_Settings.mdx)
- [User Settings](User_Settings.mdx)
- [Configuring the Self Service Portal authentication](User_Settings.mdx)
- [Setting the password complexity](User_Settings.mdx)
- [Defining the terms of service](User_Settings.mdx)
- [Configuring the User Invite reminder emails](User_Settings.mdx)
- [Configuring User Registration Confirmation emails](User_Settings.mdx)
- [Configuring User Work Schedule setting](User_Settings.mdx)
- [Configuring Admin Portal authentication setting](User_Settings.mdx)
...
- **User Registration Confirmation Setting:** Controls the ability to send the User Registration Confirmation email. See [Configuring and using registration confirmation emails](Configuring_registration_confirmation_emails.mdx)for an overview of the solution and [Configuring User Registration Confirmation emails](User_Settings.mdx) below for specific user settings instructions.
...
<WorkflowBlock>
  ...
  <WorkflowBlockItem>
    ...
    - For Supervised devices, device owner setting will be "Company Owned."
      :inlineImage[]{src="https://api.archbee.com/api/optimize/BbT7KQqqvz3LSYn2rwmao/t6a6ECVSw6tVjpYAGIUtJ_r35deviceownershipusersettings001.png" alt caption}
      Click **+Add** for at least one user group to which you want to distribute the setting.
  </WorkflowBlockItem>

  <WorkflowBlockItem>
    ([ On-demand feature](On-demand_features.mdx) for iOS and macOS devices only) Optionally, turn on the **Device Allowlist** option to allow device registration based on Allowlisted serial numbers.
  </WorkflowBlockItem>
  ...
  <WorkflowBlockItem>
    Send an invite to the users. For more information, see [Inviting Users](Inviting_Users.mdx).
  </WorkflowBlockItem>
</WorkflowBlock>
...
For users configured with SAML-based [Identity Provider](Configure_Identity_Provider.mdx) (IdP),Ivanti Neurons for MDMsupports PIN-based authentication to the administration portal. The Admin Portal Authentication Type should be PIN and Password. This feature acts as two-factor authentication for additional security. In this case, when such a user tries to log into the portal:
