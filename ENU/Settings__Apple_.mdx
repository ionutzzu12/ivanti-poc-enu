...
Administrators can turn on iOS operating system updates for automated device enrollment. If this option is enabled, then the Device Enrollment devices will use the [Software Updates](Software_Updates.mdx) configuration instead of the schedule OS update setting in the Device Enrollment Profile.
