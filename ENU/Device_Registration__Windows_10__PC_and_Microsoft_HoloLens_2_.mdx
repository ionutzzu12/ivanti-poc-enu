...
- [Manual registration](Device_Registration__Windows_10__PC_and_Microsoft_HoloLens_2_.mdx)\* [Sending an invitation](Device_Registration__Windows_10__PC_and_Microsoft_HoloLens_2_.mdx)
  - [Completing the end users registration process](Device_Registration__Windows_10__PC_and_Microsoft_HoloLens_2_.mdx)
- [Windows Autopilot](Device_Registration__Windows_10__PC_and_Microsoft_HoloLens_2_.mdx)
- [Entra ID Standard Registration](Device_Registration__Windows_10__PC_and_Microsoft_HoloLens_2_.mdx)
...
- Windows Autopilot
- With SCCM and Ivanti EPM via a Provisioning Package Enrollment with PIN. See [Provisioning Package Enrollment with PIN](Provisioning_Package_Enrollment_with_PIN.mdx).
- [Bulk Enrollment](Using_Bulk_Enrollment_for_Windows_devices.mdx)
...
- The end [user must have an account](Users.mdx) in Ivanti Neurons for MDM before you can start the device registration process. For LDAP users, that means a [Connector](Connector.mdx) and an [LDAP server](Configuring_LDAP_server.mdx) must be set up, and the user must be imported from the LDAP server. For local users, that means [adding a user](Users.mdx).
...
- in the [Startup Wizard](Getting_Started.mdx)
- when you [add one or more users](Users.mdx)
- in the Users page ([Actions > Send Invite](Inviting_Users.mdx))
...
For more information, see [Configuring Windows Autopilot Profiles](Configuring_Windows_Autopilot_Profiles.mdx).

For security and unauthorized use of the device, all Autopilot Windows devices can be locked to a tenant using the TenantLockdown CSP feature. To use this feature, the devices must be enrolled using the Autopilot option. This configuration is applied at the device level. See [TenantLockdown CSP](TenantLockdown_CSP.mdx).
