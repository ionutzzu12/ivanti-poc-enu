...
- [Requirements for enabling User Enrollment](User_Enrollment_with_Apple_Business_Manager.mdx)
- [Priority of registrations](User_Enrollment_with_Apple_Business_Manager.mdx)
- [Difference between standard MDM enrollment and User Enrollment](User_Enrollment_with_Apple_Business_Manager.mdx)
- [Difference between User Enrollment vs Device Enrollment](User_Enrollment_with_Apple_Business_Manager.mdx)
- [Connecting Ivanti Neurons for MDM to Apple Business Manager](User_Enrollment_with_Apple_Business_Manager.mdx)
...
In the [Users](Users.mdx) list page and the [Devices](Devices.mdx) list page, you can add the Managed Apple ID column to be displayed for all users. In the [Devices](Devices.mdx) list page, you can add the User Enrollment Enrolled column to display the status of User Enrollment devices. In addition, the user and device exports include these columns in CSV files.
...
| Apply supervised restrictions                                                                                              | ![](Resources/Images/X.svg)     | ![](Resources/Images/X.svg)     | :inlineImage[]{src="https://api.archbee.com/api/optimize/BbT7KQqqvz3LSYn2rwmao/DpdcJ-ljWMFDNvIRnnSnw_check.svg" alt caption}(Supervised devices only) |
...
- You must request and install an Apple [MDM certificate](Install_MDM_Certificate.mdx) to manage iOS devices.
...
1. Go to User&#x73;**>**[User Groups](User_Groups.mdx).
...
1. Go to **Users >**[User Settings](User_Settings.mdx).
...
1. Go to [Users](Users.mdx).
2. Click + **Add** > **Single User**.
   Enter the new user information and add it to the newly created user group, such as User Enrollment Group. For more information, see *Adding a user* in the [Users](Users.mdx) topic.

### Importing LDAP users to enable User Enrollment

**Prerequisites**

- As a prerequisite, set up a Ivanti Neurons for MDM connector to access [LDAP](Configuring_LDAP_server.mdx) resources.
...
1. Go to **Admin >&#xA0;**[Entra ID User Source](Connect_Ivanti_Neurons_for_MDM_with_Entra_ID_User_Source.mdx).
2. Edit the settings.
